...
function switchLocale(loc) {
  router.get(route('content-manager.disclaimer'), { content_locale: loc }, { preserveScroll: true });
}
function submit() {
  form.locale = props.contentLocale;
  form.put(route('content-manager.disclaimer.update'), { preserveScroll: true });
}
</script>

<template>
  <Head title="Disclaimer – Content manager" />
  <AuthenticatedLayout>
    <template #header>Disclaimer</template>
    <div class="admin-form-page">
      <div class="admin-form-page-header mb-3">
        <h1 class="admin-form-page-title">Disclaimer</h1>
        <p class="admin-form-page-desc text-muted small">Rich text content for the Disclaimer page on the frontend.</p>
      </div>
      ...
      <div class="admin-box admin-box-smooth mb-3">
        <CmsLocaleSelect :model-value="contentLocale" id="disclaimer-content-locale" @update:model-value="switchLocale" />
      </div>
      <div class="admin-box admin-box-smooth">
        ...
        <div class="mt-3">
          <PrimaryButton type="button" class="btn btn-primary btn-sm" :disabled="form.processing" @click="submit">Save disclaimer</PrimaryButton>
        </div>
      </div>
    </div>
  </AuthenticatedLayout>
